...
import com.lmc.shopleasing.global.CommonAttributes;
import com.lmc.shopleasing.global.EnumConverter;
import com.lmc.shopleasing.global.Setting;
import com.lmc.shopleasing.global.TemplateConfig;
...
public final class SystemUtils {
	...
	public static Setting getSetting() {
		...
		if (cacheElement == null) {
			...
			try {
				ClassPathResource classPathResource = new ClassPathResource(CommonAttributes.SHOPLEASING_XML_PATH);
				InputStream inputStream = classPathResource.getInputStream();
				Document document = new SAXReader().read(inputStream);
				List<org.dom4j.Element> elements = document.selectNodes("/shopleasing/setting");
	...
	public static void setSetting(Setting setting) {
		...
		try {
			File shopxxXmlFile = new ClassPathResource(CommonAttributes.SHOPLEASING_XML_PATH).getFile();
	...
	public static TemplateConfig getTemplateConfig(String id) {
		...
		if (cacheElement == null) {
			...
			try {
				File shopxxXmlFile = new ClassPathResource(CommonAttributes.SHOPLEASING_XML_PATH).getFile();
				Document document = new SAXReader().read(shopxxXmlFile);
				org.dom4j.Element element = (org.dom4j.Element) document.selectSingleNode("/shopleasing/templateConfig[@id='" + id + "']");
				if (element != null) {
	...
	public static List<TemplateConfig> getTemplateConfigs(TemplateConfig.Type type) {
		...
		if (cacheElement == null) {
			...
			try {
				File shopxxXmlFile = new ClassPathResource(CommonAttributes.SHOPLEASING_XML_PATH).getFile();
				Document document = new SAXReader().read(shopxxXmlFile);
				List<org.dom4j.Element> elements = document.selectNodes(type != null ? "/shopleasing/templateConfig[@type='" + type + "']" : "/shopxx/templateConfig");
				for (org.dom4j.Element element : elements) {
